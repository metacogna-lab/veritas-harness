/**
 * Research dogma — 8 epistemic gates every research plan must satisfy.
 * Copied from harness/veritas-example/src/config/dogma.ts (pure predicates, no harness imports).
 */
...
export const DEFAULT_DOGMA: DogmaDimension[] = [
  {
    ...
    check(plan) {
      ...
      if (VAGUE_VERBS.test(plan.objective) && plan.objective.length < 60)
        return { pass: false, reason: "objective starts with a vague verb and is not sufficiently specific" };
  ...
  },
  {
    ...
    check(plan) {
      ...
      if (!hasPaths && !hasHosts)
        return { pass: false, reason: "scope.paths and scope.hosts are both empty — research has no bounded domain" };
      return { pass: true, reason: `scope bounded: ${hasPaths ? `paths=${plan.scope.paths.join(",")}` : `hosts=${plan.scope.hosts.join(",")}`}` };
    },
  },
  {
    id: "phased-approach",
    ...
    check(plan) {
      if (plan.phases.length < 2)
        return { pass: false, reason: `only ${plan.phases.length} phase(s) defined — depth requires ≥ 2 distinct phases` };
  ...
  },
  {
    ...
    check(plan) {
      ...
      if (measurable.length === 0)
        return { pass: false, reason: "no success criterion contains measurable language (e.g. 'at least N', 'verify', 'pass@1', 'reproduce')" };
  ...
  },
  {
    ...
    check(plan) {
      if (plan.sources.length === 0)
        return { pass: false, reason: "no sources provided — research should cite at least one document or lesson" };
    ...
    check(plan) {
      for (const phase of plan.phases) {
        if (DECEPTIVE_PHASE_WORDS.test(phase.description))
          return { pass: false, reason: `phase "${phase.id}" contains language suggesting hidden intent: "${phase.description.slice(0, 80)}"` };
      }
  ...
  },
  {
    ...
    check(plan) {
      ...
      if (unfocused.length > 0)
        return { pass: false, reason: `${unfocused.length} specialist(s) have an under-specified focus (<10 chars)` };
  ...
  },
  {
    ...
    check(plan) {
      ...
      if (selfReport.length > 0)
        return { pass: false, reason: "some success criteria rely on model self-report rather than external evidence" };
