...
description: >-
  ...
  "evaluate the harness", "add an eval", "measure pass@1", run bench, verify
  reported capability numbers against a committed oracle, or check reproducibility.
---
...
2. To add a suite:
   ...
   - Create `bench/<suite>/solver.mjs` exporting `solve(input)` — **task-agnostic** (no task
...
- After adding a tool via **harness-tool-adder**, add a suite that exercises it here.
- **harness-refuter** uses committed bench evidence when verifying findings from eval results.
- Use **harness-analysis** to summarize eval results across harnesses into a research report.

## Standing rules

- Grading logic must not reference specific test answers (anti-fitting guard fails the build).
